...
public class SchemaBrowser extends JTree {
    ...
    private static class ObjectNode extends DynamicUtilTreeNode {
        ...
        private void addChildren() throws SQLException {
            switch (getLevel()) {
                ...
                case 3:
                    ...
                    if (connectionData.getConnection().getMetaData().supportsSchemasInTableDefinitions()) {
                        addQueryColumn(connectionData.getConnection().getMetaData()
        ...
        private void addQueryColumn(ResultSet resultSet, boolean children, int columnIndex) throws SQLException {
            while (resultSet.next()) {
                add(resultSet.getString(columnIndex) + " " + resultSet.getString("TYPE_NAME")
                  + "(" + resultSet.getString("COLUMN_SIZE")
                     + ") " + ("0".equals(resultSet.getString("NULLABLE")) ? "NOT NULL" : "NULL"), children);
            }
        }
